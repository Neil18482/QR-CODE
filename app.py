...
# Secret key for AES encryption (32 bytes key for AES-256)
...
def normalize_name(name):
    ...
    # ✅ Normalize spaces, dots, and case
...
def is_valid_student(student_id):
    """
    Checks if the Student ID exists in students.csv.
    Ignores the name completely.
    """
    students = read_students()  # Load students.csv data

    if student_id in students:
        return True  # ✅ Student ID is valid
    else:
        print(f"❌ ERROR: Student ID {student_id} not found in students.csv!")
        return False  # ❌ Reject invalid Student ID
...
# ✅ Download Attendance Report (Grouped by Year & Section)
